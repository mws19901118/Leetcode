    ...
    def search(self, nums: List[int], target: int) -> int:
        ...
        while start <= end:
            ...
            if nums[mid] == target:                                                                                                 #If found target, return mid.
                return mid
            if nums[start] <= nums[mid] < target or nums[mid] < target <= nums[end] or target < nums[start] <= nums[mid]:           #1. If nums[mid] is smaller than target and both nums[mid] and target is on same slope, target is in the second half(between mid and end).

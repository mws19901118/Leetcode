    ...
    def minimumDeleteSum(self, s1: str, s2: str) -> int:
        ...
        for i, j in product(range(m + 1), range(n + 1)):                                           #Traverse s1 and s2.
            if not i and not j:                                                                    #If i == 0 and j == 0, s1[:i] and s2[:j] are 2 empty strings, so no action needed.
                continue
            elif not i:                                                                            #If i == 0 and j > 0, s1[:i] is empty, to make them equal, delete all character in s2[:j].
                dp[i][j] = dp[i][j - 1] + ord(s2[j - 1])
            elif not j:                                                                            #If i > 0 and j == 0, s3[:j] is empty, to make them equal, delete all character in s1[:i].

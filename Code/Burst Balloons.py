class Solution(object):
    def maxCoins(self, nums):
        ...
        l += 2                                            #Calculate the new length.
        dp = [[0 for j in range(l)] for i in range(l)]    #Record the max sum of coins collected from nums[i] to nums[j](consider nums[i] and nums[j] unreal).
        for i in range(2, l):                             #Examine every length of intevals from i to j; the minimum length is 2.
            for j in range(0, l - i):                     #Examine every intevals of current length.
        ...
        return dp[0][l - 1]                               #Return the coins collected from nums[-1] to nums[n].

...
class Solution:
    ...
    def possibleBipartition(self, N: int, dislikes: List[List[int]]) -> bool:
        ...
        color = {}
        return all(self.DFS(graph, color, i, 0) for i in range(1, N + 1) if i not in color)     For each connected component, we can check whether it is bipartite by just trying to coloring it with two colors.

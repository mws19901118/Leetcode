class Solution:
    def subarraysDivByK(self, nums: List[int], k: int) -> int:
        ...
        count = collections.Counter(s)                          #For each pair of indexes, i and j, if s[i] = s[j], then s[i] + (nums[i] + ... + nums[j]) % k = s[j].
        return sum(v * (v - 1) // 2 for v in count.values())    #Thus, nums[i] to nums[j] is a subarray whose sum is divisible by k.

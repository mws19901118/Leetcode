class Solution:
    def shortestWay(self, source: str, target: str) -> int:
        indexes = collections.defaultdict(list)                             #Store the indexes of each letter in source.
        ...
        for x in target:                                                    #Traverse target.
            if x not in indexes:                                            #Return -1 if x not in indexes, meaning x not in source as well.
                return -1
            index = bisect.bisect_left(indexes[x], prevIndex + 1)           #Find the first index of x which id greater than prevIndex.

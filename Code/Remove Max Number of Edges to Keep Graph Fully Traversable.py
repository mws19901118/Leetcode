...
class Solution:
    def maxNumEdgesToRemove(self, n: int, edges: List[List[int]]) -> int:
        ...
        result = sum(int(int(ufs1[x - 1].union(ufs1[y - 1])) + int(ufs2[x - 1].union(ufs2[y - 1])) == 0) for x, y in type3)                                                                 #Traverse type3 edges first, try yo union nodes for both alice and bob; if both alice nodes and bob nodes don't need union, increase count as current edge is not needed.

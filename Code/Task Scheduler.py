class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        count = list(Counter(tasks).values())                               #Count each task.
        ...
        return max(len(tasks), (n + 1) * (maxNum - 1) + maxNumCount)        #If there is no idle CPU cycle, the total time needed is the length of tasks.

...
class Solution:
    def mincostToHireWorkers(self, quality: List[int], wage: List[int], K: int) -> float:
        ...
        minWage = qualitySum * worker[K - 1][0]                         #The wage for pay group of first K workers is the total quality multiplied by the largest ratil in pay group. Since we sort workers by ratio, so it's qualitySum * worker[K - 1][0].

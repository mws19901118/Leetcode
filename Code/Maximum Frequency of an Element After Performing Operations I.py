class Solution:
    def maxFrequency(self, nums: List[int], k: int, numOperations: int) -> int:
        ...
        for i in range(n + 1):                                                  #Traverse from 0 to n.
            ...
            result = max(result, count[i] + min(numOperations, left + right))   #Since the operation has a limit, we can update total min(numOperations, left + right) numbers to i. Thus update result if count[i] + min(numOperations, left + right) is larger.
        return result

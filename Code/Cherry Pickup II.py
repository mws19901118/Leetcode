    ...
    def cherryPickup(self, grid: List[List[int]]) -> int:
        ...
        for i in range(1, m):                                                           #Traverse top down.
            ...
            for (x, y), val in cherryDict.items():
                robot1 = [i for i in [x - 1, x, x + 1] if i >= 0 and i < n]             #Generate all available columns for robot 1 in this row.
                robot2 = [i for i in [y - 1, y, y + 1] if i >= 0 and i < n]             #Generate all available columns for robot 2 in this row.
                for a in robot1:
                    for b in robot2:

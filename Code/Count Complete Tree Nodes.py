...
class Solution:
    def countNodes(self, root: Optional[TreeNode]) -> int:
        ...
        while r.right is not None:                                                                                  #Because it's complete treem the length of the right most branch is the height of right tree.
            r = r.right
            rh += 1
        return pow(2, lh + 1) - 1 if lh == rh else 1 + self.countNodes(root.left) + self.countNodes(root.right)     #If left height equals right height, it's a perfect tree and return 2 ^ (lh + 1) - 1; otherwise, it's not a perfect tree, calculate number of nodes recursibely.

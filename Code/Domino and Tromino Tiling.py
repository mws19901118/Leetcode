class Solution:
    def numTilings(self, n: int) -> int:
        ...
        dp1, dp2, dp3 = 1, 2, 5                                       #Initialize DP value for the last 3 elements; inititally for n = 1, 2, 3.
        for i in range(3, n):                                         #Compute the DP value, from 4 to n; suppose the DP function is F(), then the dp value is F(x).
        ...
        return dp3                                                    #We can also use 2 trominos at both end and any number of dominos(including 0) in the middle to form tiling whose length is longer than or equal to 3.
                                                                      ...
                                                                      #And 2 new tilings for length x ifself.
                                                                      #Thus, F(x) = F(x - 1) + F(x - 2) + (F(1) + ... + F(x - 3) + 1) * 2.
                                                                      #In same way, F(x + 1) = F(x) + F(x - 1) + (F(1) + ... + F(x - 2) + 1) * 2 = F(x) + (F(x - 1) + F(x - 2) + (F(1) + ... + F(x - 3) + 1) * 2) + F(x - 2) = F(x) * 2 + F(x - 2).

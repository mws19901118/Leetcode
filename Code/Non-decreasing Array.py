class Solution:
    def checkPossibility(self, nums: List[int]) -> bool:
        ...
        while j < len(nums) and nums[j] >= nums[j - 1]:
            j += 1
        return j == len(nums) and (i == 1 or i + 1 == len(nums) or nums[i] >= nums[i - 2] or nums[i + 1] >= nums[i - 1])          #Also, if b is start or c is end or c >= a or d >= b, we can set b = c; only under these 4 conditions can we make the whole array non-decreasing.

class Solution:
    def stoneGameII(self, piles: List[int]) -> int:
        @cache                                                                              #Cache result.
        def dp(index: int, m: int, isAlice: bool) -> int:                                   #Use dp to calculate the max stones Alice can get for piles[index + 1:] and m and if the player is Alice.
            if index >= len(piles) - 1:                                                     #If index >= len(piles) - 1, it reaches the end, so return 0.
                return 0
            result = 0 if isAlice else inf                                                  #Initialize result, if player is alice, we will return current max value; otherwise, return current min value.

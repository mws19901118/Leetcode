...
class Solution:
    def validTree(self, n: int, edges: List[List[int]]) -> bool:
        ...
        return len(edges) == n - 1 and not any(not ufs[x].union(ufs[y]) for x, y in edges) #The number of edges should be exactly n - 1; also there shouldn't be any edge connecting 2 nodes that are already in same component.

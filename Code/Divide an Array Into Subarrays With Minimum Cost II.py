    ...
    def minimumCost(self, nums: List[int], k: int, dist: int) -> int:        
        ...
        for i in range(1, len(nums) - dist - 1):                     #Traverse from 1 to the last possible index of the start of sliding window.
            ...
            if index < k - 1:                                        #If the index is smaller than k - 1, meaning the number moved out of the sliding window is in top k - 1 smallest.
                ...
                if rightIndexInSortedList < k - 1:                   # If added number is in top k - 1, add it to s.
                    s += nums[right]
                else:                                                #Otherwise, we need to add what's now at position k - 2((was at k - 1 before)) in sorted list to s.
                    s += sl[k - 2]
            elif if rightIndexInSortedList < k - 1:                  #If the index is not smaller than k - 1 but the added number is in top k - 1, we need to add it to s then decrease the current number at k - 1 from s.
                  s += nums[right] - sl[k - 1]
                  s -= sl[k - 1]

class Solution:
    def minMeetingRooms(self, intervals: List[List[int]]) -> int:
        points = sorted([(x, 1) for x, y in intervals] + [(y, -1) for x, y in intervals])           #Break intervals to points, 1 for start point and -1 for end point and sort in asending order.

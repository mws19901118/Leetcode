...
#         self.right = right
class Solution:
    def dfs(self, node: TreeNode) -> Tuple:                 #DFS, return a tuple of min cameras needed in 3 situations(all nodes covered except node, all nodes covered covered and no camera placed at node, all nodes covered and camera placed at node).
